...
Для ознакомления и запуска примеров рекомендуется использовать Pycharm, поскольку только в этой среде
осуществлялась проверка работоспособности. Создаем проект с любым именем. С помощью команды, приведенной 
выше, устанавливаем библиотеку **peacepie** в проект. Создаем скрипт, приведенный ниже, и запускаем его 
первый раз без параметров. При запуске скрипта без параметров библиотека формирует окружение по умолчанию.
При повторных запусках скрипт лучше запускать с параметром **"./config/peacepie.cfg"**.
При первоначальной инициализации происходит скачивание более десятка библиотек из удаленных репозиториев,
что занимает некоторое время. Дождитесь пока в окне терминала не появиться сообщение от утилиты **pip**: 
"Successfully installed lib1, lib2, ...". 
...
содержательный ответ от исполнителя. Единственное исключение, приходящее на ум,- команда
"exit", по которой система незамедлительно закрывается, ответ не предусматривается и
доставка становится невозможной в закрывающейся системе.
...
| Тело       | {"class_desc": {"package_name": "peacepie_example", "class": "HelloWorld"}, "name": "hello_world"} |
| Получатель | first.main.admin                                                                                   |

В результате создастся новый актор типа HelloWorld из пакета **peacepie_example**, имя можно 
задать любое, пакет, если он еще не кеширован на локальной машине, будет скачан из удаленного репозитория. 
Если отправить любую команду в адрес созданного актора, используя его имя, кроме
команды "exit", то созданный актор будет выводить в консоль сообщение "Hello, World!".
...
Следующий актор похож на предыдущий, но выводит сообщение "Hello, World!" в консоль с заданной 
периодичностью после получения команды "start". Кроме периодичности можно еще задать лимит сообщений.
При заданном лимите актор выведет ограниченное число сообщений.


| Поле       | Значение                                                                                                              |
|------------|-----------------------------------------------------------------------------------------------------------------------|
| Команда    | create_actor                                                                                                          |
| Тело       | {"class_desc": {"package_name": "peacepie_example", "class": "IteratingHelloWorld"}, "name": "iterating_hello_world"} |
| Получатель | first.main.admin                                                                                                      |
...
Остановить вывод можно с помощью команды "stop"
...
Следующий пример более интересный. Запускаем программу заново и создаем актор типа CircleStarter

| Поле        | Значение                                                                                                 |
|-------------|----------------------------------------------------------------------------------------------------------|
| Команда     | create_actor                                                                                             |
| Тело        | {"class_desc": {"package_name": "peacepie_example", "class": "CircleStarter"}, "name": "circle_starter"} |
| Получатель  | first.main.admin                                                                                         |

Далее одновременно даем команду на старт созданному актору и задаем параметры.
 
| Поле        | Значение                           |
|-------------|------------------------------------|
| Команда     | start                              |
| Тело        | {"process_count": 3, "period": 1}  |
...
|-------------|---------------------------------------------------------------------------------------------------|
| Команда     | create_actor                                                                                      |
| Тело        | {"class_desc": {"package_name": "peacepie_example", "class": "CircleDancer"}, "name": "dancer_x"} |
