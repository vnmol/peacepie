...
class PackageLoader:
    ...
    async def load_package(self, msg):
        ...
        dir_opers.makedir(self.tmp_path, clear=True)
        url_key = 'index-url'
        index_url = body.get(url_key) if body.get(url_key) else params.instance.get(url_key)
        entry = await self.acquire_package(requires_dist, index_url, timeout)
        if entry:
            self.move_packages()
            body = {'entry': entry}
    ...
    async def acquire_package(self, requires_dist, index_url, timeout):
        ...
        await self.install_package(package_name, version_spec, index_url, timeout)
        ...
        if not ver:
            return None
        bundle = []
        tasks = [asyncio.create_task(self.acquire_package(dependency, None, timeout))
